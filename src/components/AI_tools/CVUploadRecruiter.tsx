...
export default function CVUploadRecruiter({ recruiter }: CVUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5678/webhook-test/upload-cv-recruiter', {
        method: 'POST',
        body: formData,
      });
    ...
    // console.log('CV Link:', result["file"]);

    // try {
    //   console.log('Saving CV form data to MySQL:', cvData);
    ...
    //   const result_cv = await response.json();
    //   console.log("API Response:", result_cv);

    //   if (response.ok) {
    //     // setJobPosted(true);
    //     console.log("CV saved successfully:", result_cv);
    //   } else {
    //     console.error('Failed to save CV:', result_cv);
    //     alert(`Failed to save CV: ${result_cv.error || result_cv.message || 'Unknown error'}`);
    //   }
    // } catch (error) {
    ...
    try {
      ...
      const response = await fetch('/api/aiAnalysisRecruiter', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
